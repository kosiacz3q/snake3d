	...
	for(MapObjects::const_iterator obj_iter = objects.begin() ; obj_iter != objects.end(); ++obj_iter){
		for(Tail::const_iterator tail_iter = obj_iter->second->tail.begin() ; tail_iter != obj_iter->second->tail.end() ; ++tail_iter ){
			...
			switch( (int)tail_iter->position.w){
				case 1:
					glTranslatef( ( tail_iter->position.x - wallDim[0][2] ) * objectsBrickDimm ,
								  (wallDim[0][3] - tail_iter->position.y) * objectsBrickDimm ,
				...
				case 2:
					glTranslatef(halfOfMainBrick + halfOfObjectsBrick + (tail_iter->position.z * objectsBrickDimm),
								(wallDim[1][3] - tail_iter->position.y) * objectsBrickDimm ,
								( tail_iter->position.x - wallDim[1][2] ) * objectsBrickDimm
								);
					break;
				case 3:
					glTranslatef( ( tail_iter->position.x - wallDim[2][2] ) * objectsBrickDimm ,
								  (wallDim[2][3] - tail_iter->position.y) * objectsBrickDimm ,
								   -halfOfMainBrick - halfOfObjectsBrick - (tail_iter->position.z * objectsBrickDimm));
					break;

				case 4:
					glTranslatef(-halfOfMainBrick - halfOfObjectsBrick - (tail_iter->position.z * objectsBrickDimm),
								(wallDim[3][3] - tail_iter->position.y) * objectsBrickDimm ,
								( tail_iter->position.x - wallDim[3][2] ) * objectsBrickDimm
								);
					break;
		...
		case Message::MOVE_REQUEST:{
			...
			case 1:{
				...
				switch(mvr->direction){
					case Direction::DOWN : mvr->head.position.y += 1;break;
					case Direction::UP : mvr->head.position.y -= 1;break;
					case Direction::RIGHT : mvr->head.position.x += 1;break;
					case Direction::LEFT : mvr->head.position.x -= 1;break;
					default: break;
				}
			...
			//setting color
			switch( (int)mvr->head.position.w){
			case 1: mvr->head.color = Colors::Blue;break;
			case 2: mvr->head.color = Colors::Black;break;
			case 3: mvr->head.color = Colors::Cyan;break;
			case 4: mvr->head.color = Colors::Orange;break;
			case 5: mvr->head.color = Colors::LimeGreen;break;
			case 6: mvr->head.color = Colors::Violet;break;
			}
